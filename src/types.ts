...
export interface SoilLabResults {
  ...
  ph_cacl2?: number;      // pH CaCl2
  ph_h2o?: number;        // pH H2O
  ph_kcl?: number | string; // pH KCl (aceita numérico ou "ns")
  mo?: number;            // M.O. (g/dm³)
  p_meh?: number;         // P meh (mg/dm³)
  p_res?: number;         // P res (mg/dm³)
  p_rem?: number | string; // P rem (mg/dm³, aceita "ns")
  k?: number;             // K+ (mmolc/dm³)
  ca?: number;            // Ca 2+ (mmolc/dm³)
  mg?: number;            // Mg 2+ (mmolc/dm³)
  al?: number;            // Al 3+ (mmolc/dm³)
  h_al?: number;          // H+Al (mmolc/dm³)
  sb?: number;            // SB (Soma de Bases) (mmolc/dm³)
  ctc_t?: number;         // CTC (T) total (mmolc/dm³)
  v_percent?: number;     // V% (Saturação bases) (%)
  s?: number;             // S (Enxofre) (mg/dm³)
  ca_mg?: number;         // Relação Ca/Mg (saturação/relação)
  ca_k?: number;          // Relação Ca/K (saturação/relação)
  mg_k?: number;          // Relação Mg/K (saturação/relação)
  b?: number;             // B (Boro) (mg/dm³)
  cu?: number;            // Cu (Cobre) (mg/dm³ Mehlich)
  fe?: number;            // Fe (Ferro) (mg/dm³ Mehlich)
  mn?: number;            // Mn (Manganês) (mg/dm³ Mehlich)
  zn?: number;            // Zn (Zinco) (mg/dm³ Mehlich)
  ca_t?: number;          // Ca/T (%)
  mg_t?: number;          // Mg/T (%)
  k_t?: number;           // K/T (%)
  argila?: number;        // Argila (%)
  silte?: number;         // Silte (%)
  areia_total?: number;   // Areia Total (%)
...
export const FERTILITY_THRESHOLDS: Record<keyof SoilLabResults, { low: number; medium: number; high: number; unit: string; name: string }> = {
  ...
  s: { low: 5, medium: 10, high: 15, unit: 'mg/dm³', name: 'S' },
  ...
  k_t: { low: 2, medium: 4, high: 6, unit: '%', name: 'K/T' },
